# -*- coding: utf-8 -*-
"""
Created on Wed Dec 21 10:21:49 2016
...
with open('min_K_01_2p', 'wr') as res : 
    for kappa in np.arange(0.5, 2., 0.5) :
        f = lambda x : Energia(0, 5, [x[0],0], [x[1],0],0,0,kappa, Ns, 'K01')
        x0 = [0.2,0.2]
        minim = opt.minimize(f, x0, method='Nelder-Mead', tol=1e-6)
        Q_minimal = minim.x[0]
        F1_minimal = minim.x[1]
        mu_minimal  = Bis(0,5,[Q_minimal,0],[F1_minimal,0],0, 0, kappa, Ns, 'K01')
